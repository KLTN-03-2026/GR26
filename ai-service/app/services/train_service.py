...
def train_branch_model(
    ...
    Returns:
        dict với keys: mae, mape, model_path, series_count

    Raises:
        ValueError: Data không hợp lệ sau filter
        RuntimeError: Train thất bại
    """
    # Bước 1: Lọc series đủ điều kiện
    ...
    n_rows = len(df)
    logger.info(
        "Train branch: tenant=%s branch=%s | %d series | %d rows | n_lags=%d | yearly=%s",
        tenant_id, branch_id, n_series, n_rows, n_lags, yearly_seasonality,
    )

    # Bước 2: Tạo model với config của branch
    ...
    # Bước 4a: Lấy MAE từ metrics cuối epoch
    mae: float | None = None
    try:
    ...
    # Bước 4b: Tính MAPE từ in-sample predictions
    # MAPE = mean(|actual - predicted| / (actual + 1)) × 100
    ...
    try:
        ...
        if "yhat1" in pred_df.columns and "y" in pred_df.columns:
            ...
            if not valid.empty:
                mape = float(
                    ((valid["yhat1"] - valid["y"]).abs() / (valid["y"] + 1)).mean() * 100
                )
                logger.info("MAPE (in-sample): %.2f%%", mape)
    except Exception:
        logger.warning("Không tính được MAPE — bỏ qua")

    # Bước 5: Lưu model kèm config (bao gồm n_lags đã dùng) để validate khi predict
