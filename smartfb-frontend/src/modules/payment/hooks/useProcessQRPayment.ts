...
export const useProcessQRPayment = () => {
  // Author: Hoàng | date: 2026-05-04 | note: invalidate tables để máy thực hiện thanh toán tự cập nhật thẻ về sẵn sàng ngay
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (payload: ProcessQRPaymentRequest) =>
      paymentService.processQRPayment(payload),
    // Author: Hoàng | date: 2026-05-04 | note: sau QR payment thành công, refetch thẻ để đồng bộ trạng thái AVAILABLE
    onSuccess: () => {
