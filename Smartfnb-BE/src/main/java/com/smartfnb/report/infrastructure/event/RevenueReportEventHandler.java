...
public class RevenueReportEventHandler {
    ...
    public void onOrderCostCalculated(OrderCostCalculatedEvent event) {
        ...
        try {
            ...
            dailyRevenueSummaryRepo.initIfNotExists(UUID.randomUUID(), event.tenantId(), event.branchId(), event.date());

            // Khoá bi quan để ngăn lost update
            Optional<DailyRevenueSummary> existing = dailyRevenueSummaryRepo.findByBranchIdAndDateForUpdate(event.branchId(), event.date());
    ...
    private void updatePaymentBreakdown(UUID tenantId, UUID branchId, LocalDate date,
        ...
        dailyRevenueSummaryRepo.initIfNotExists(UUID.randomUUID(), tenantId, branchId, date);
        Optional<DailyRevenueSummary> existing = dailyRevenueSummaryRepo.findByBranchIdAndDateForUpdate(branchId, date);
    ...
    private void updateHourlyRevenueStat(UUID branchId, LocalDate date, int hour,
        ...
        HourlyRevenueStat stat = new HourlyRevenueStat(
            UUID.randomUUID(), branchId, date, hour,
            orderCount, revenue
        );
        hourlyRevenueStatRepo.upsert(stat);
    }
